...
for ii = 4
  ...
  [As, index_pairs] = data_split_dynamic_20170108_4(origin_A, 5);
  ...
  for jj = length(As):-1:1
    ...
    if data_length > 10 * merged_num
      ...
      [delta, fi, arc, omega] = oval_find_T_20170108(result(1),result(2),result(3),result(4),result(5),oXs,oYs,period);
	...
	text(origin_A(3,1),origin_A(4,1)+8,strcat('Contained in:', int2str(contained(jj))),'Color','red','FontSize',14);
      ...
      fprintf(fid, 'total_id, %d, data_file,%s,file_id,%d,part_id,%d,start_point,%d,end_point,%d,point_num,%d,object,%f,a,%f,b,%f,h,%f,k,%f,theta,%f,delta_T,%f,arc,%f,T,%f,fi,%f,omega,%f,merged,%d,local_max_num,%d,consider,%d,contained,%d,consider2,%d\n', ...
	      Vehicle, input_filename(1:end-1), ii, jj, index_pairs(jj,1), index_pairs(jj,2), point_length, object, result(1),result(2),result(3),result(4),result(5),delta,arc,T,fi,omega,merged_num,local_max_num,consider(jj),contained(jj),consider2(jj));
