...
%% output
out_file_dir_parent = '/home/nukuok/Desktop/baking/script_for_new_data/result_20170128_2/';
...
for ii = 1:32
  ...
  [As, index_pairs] = data_split_dynamic_20170128(origin_A, 5, png_name);
  % box on;
  % grid on;
  % grid minor;
  ...
  fig = figure(1);
  set(fig,'Position', [100,100,575,375]);
  ...
  p1 = plot([75,75],[-10,50],'color',[0.4,0.4,0.9],'linewidth',2);
  p2 = plot([80,80],[-10,50],'--','color',[0.4,0.4,0.9],'linewidth',2);
  plot([85,85],[-10,50],'color',[0.4,0.4,0.9],'linewidth',2);
  legend([p2,p1], 'preceding car''s speed', 'light stimulus'' speed', 'Location', 'southwest');
  legend boxoff;

  plot(origin_A(4,:),origin_A(3,:),'y.','linewidth',2)
  plot(origin_A(4,1),origin_A(3,1),'bv','linewidth',3)
  ...
  figure(1);
  maing = gca;
  for mamekk2 = 1:length(to_plot_oval_center)
